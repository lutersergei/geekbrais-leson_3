...
while ($i<=$n)
{
    if (($i%3)===0)                 //Ноль делится на 3 без остатка, поэтому оставил его в решении
    {
        echo "{$i},";
    }
    $i++;
...
foreach ($Russia as $region => $cities)                                     //Не довел до ума. Работает неправильно
{
    if ((count($cities)>0))
    {
       echo "<p> $region: </p>";
...
//$array=preg_split('//',$string,-1, PREG_SPLIT_NO_EMPTY);      //не заработало
